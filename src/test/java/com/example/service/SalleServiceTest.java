...
public class SalleServiceTest {
    ...
    public void testCrudOperations() {
        // Create
        Salle salle = new Salle("Demo Room", 20); // sample room
        salle.setDescription("Demo setup");
        salle.setEtage(1);
        ...
        assertEquals("Demo Room", foundSalle.get().getNom());
    ...
    public void testFindByDisponible() {
        // Create test rooms
        Salle s1 = new Salle("Open Room", 20);
        s1.setDisponible(true);

        Salle s2 = new Salle("Closed Room", 30);
        ...
        assertTrue(availableRooms.stream().anyMatch(s -> s.getNom().equals("Open Room")));
        assertFalse(availableRooms.stream().anyMatch(s -> s.getNom().equals("Closed Room")));

        List<Salle> unavailableRooms = service.findByDisponible(false);
        assertTrue(unavailableRooms.stream().anyMatch(s -> s.getNom().equals("Closed Room")));
    ...
    public void testFindByCapaciteMinimum() {
        // Create test rooms
        Salle s1 = new Salle("Compact Room", 10);
        Salle s2 = new Salle("Standard Room", 50);
        Salle s3 = new Salle("Wide Room", 100);
        ...
        assertEquals(2, roomsMin50.stream()
                .filter(s -> s.getNom().equals("Standard Room") || s.getNom().equals("Wide Room"))
                .count());

        List<Salle> roomsMin80 = service.findByCapaciteMinimum(80);
        assertEquals(1, roomsMin80.stream()
                .filter(s -> s.getNom().equals("Wide Room"))
                .count());
